...
export const SendTransferScreen: React.FC<SendTransferScreenProps> = ({
  ...
  const getGasLimitETH = async () => {
    setIsLoading(true)
    await calculateGasLimitETH(mainAddress, state.to, transferValue).then((response) => {
      console.log(response);
      sendETH(response);
    }).catch((error) => {
      console.log(error);
      if (error = 'invalid argument 0: hex string has length 0, want 40 for common.Address') {
        Toast.show('Please insert a valid Ethereum address to continue', Toast.SHORT);
      }
      setIsLoading(false)
    });
  };
          ...
          try {
              Wallet.keystore.keyFromPassword(password, async (err, pwDerivedKey) => {
                  if (!err) {
                      ...
                      let nonce = 0;
                      Wallet.web3.eth.getTransactionCount(from, 'pending', async (err, res) => {
                        txOptions.nonce = res;
          ...
          <TransfersContainer>
            ...
            {!isNaN(transferValue) && transferValue ? (
              <FeeText style={{textTransform: 'uppercase'}}>
                {transferValue} {currency.type}= ${transferValue * currency.value.usd}
              </FeeText>
            ) : null}
          </TransfersContainer>
        </Header>
